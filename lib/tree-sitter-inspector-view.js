...
class TreeSitterInspectorView {
  getTitle () {
    return 'Tree Sitter Inspector';
  }

  constructor (serializedState) {
    console.log('new TreeSitterInspectorView', serializedState);
  ...
  compileQuery (contents) {
    let language = this.languageMode.rootLanguageLayer.language;
  ...
  attachToEditor (editor) {
    ...
    this.markerLayer = this.editor.addMarkerLayer({ role: 'tree-sitter-inspector' });
    ...
    if (this.scmGrammar) {
      ...
      this.queryEditorMarkerLayer = this.queryEditor.addMarkerLayer({ role: 'tree-sitter-inspector' });
  ...
  async update () {
    let tree = this.languageMode?.rootLanguageLayer?.tree;
    if (!tree) {
      console.error('No tree!');
      return;
    }
    let { showAnonymousNodes } = this;
    await this.renderTree(tree, {
      showAnonymousNodes
    });
  ...
  highlightActiveNode ({ scroll = true } = {}) {
    // console.log('highlightActiveNode scroll:', scroll);
    // console.trace();
    ...
    if (this.activeNodeId) {
      ...
      if (this.treeRowHighlightedIndex !== -1) {
        let row = this.treeRows[this.treeRowHighlightedIndex];
        if (row) {
          this.treeRows[this.treeRowHighlightedIndex] = row.replace('plain', 'highlighted');
        }
      }
    }
    ...
    if (this.treeRowHighlightedIndex !== -1) {
      ...
      //   scrollTopTarget = offset - 20;
      // } else if (scrollTop < ((offset + lineHeight + 40) - containerHeight)) {
      //   scrollTopTarget = (offset - containerHeight + 40);
      // } else {
      //   scrollTopTarget = offset;
      // }
  ...
  get tree () { return this.languageMode?.rootLanguageLayer?.tree; }
  ...
  handleCursorMovement () {
    ...
    // let last = this.output.querySelector('a.highlighted');
    // if (last) {
    //   last.classList.replace('highlighted', 'plain');
    // }
  ...
  handleTreeClick (event) {
    if (event.target.tagName === 'A') {
      ...
      console.log(`getting node ${nodeId} at position`, startRow, startColumn);
      let node = this.languageMode.getSyntaxNodeAtPosition(
        new Point(startRow, startColumn),
        (node) => {
          console.log('is it this one?', node);
  ...
  render () {
    let scrollStyle = '';
    // let preBackgroundColor = this.preElementComputedStyle?.getPropertyValue('background-color');
    // if (preBackgroundColor) {
    //   scrollStyle = `background-color: ${preBackgroundColor}`;
    // }
    // let scrollStyle = `background-color: ${this._preBackgroundColor || 'transparent'}`;
    ...
    let result = (
      $.div({ ref: 'element', className: 'tree-sitter-inspector pane-item' },
