...
function getCaptures (query, node, from, to) {
  if (query.captures.length === 4) {
    // Old function signature.
    return query.captures(node, from, to);
  } else {
    // New function signature that matches `node-tree-sitter`.
    return query.captures(node, { startPosition: from, endPosition: to });
  }
}
...
class TreeSitterInspectorView {
    ...
    for (let i = 0;; i++) {
      ...
      if (visitedChildren) {
        ...
        if (displayName) {
          ...
          let fieldName = getFieldName(cursor);
          if (fieldName) {
  ...
  runTreeQuery (query, startRow = null, endRow = null) {
    ...
    for (let capture of captures) {
      ...
      let marker = this.markerLayer.markBufferRange(
        rangeForNode(range),
        { invalidate: 'touch' }
      );
